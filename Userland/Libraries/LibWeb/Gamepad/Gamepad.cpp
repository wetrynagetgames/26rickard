...
JS_DEFINE_ALLOCATOR(Gamepad);

WebIDL::ExceptionOr<JS::NonnullGCPtr<Gamepad>> Gamepad::create(JS::Realm& realm, NonnullRefPtr<Core::Gamepad> gamepad)
{
    return realm.heap().allocate<Gamepad>(realm, realm, gamepad);
}
...
WebIDL::ExceptionOr<Vector<JS::NonnullGCPtr<GamepadButton>>> const Gamepad::buttons()
{
    // TODO: This not what the spec does
    Vector<JS::NonnullGCPtr<GamepadButton>> buttons;
