...
public:
    ...
    WebIDL::ExceptionOr<Vector<JS::GCPtr<Gamepad>>> get_gamepads();
    WebIDL::ExceptionOr<void> register_new_gamepad(String const& path);

private:
    WebIDL::ExceptionOr<JS::GCPtr<Gamepad>> construct_gamepad(AK::NonnullRefPtr<Core::Gamepad> underlaying_gamepad);
    int select_unused_gamepad_index();

    bool m_loaded_initial_gamepads { false };
    Vector<JS::GCPtr<Gamepad>> m_gamepads;
