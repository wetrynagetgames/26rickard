...
{
    auto property_id = PropertyID::Invalid;
    if (property_name.starts_with("--"sv)) {
        property_id = PropertyID::Custom;
    } else if (auto maybe_property_id = property_id_from_string(property_name); maybe_property_id.has_value()) {
        property_id = maybe_property_id.value();
    } else {
        return {};
    }
    VERIFY(property_id != PropertyID::Invalid);
